...
public class HttpClientUtil {
    ...
    private static void buildHttpClient() {
        ...
        RequestConfig globalConfig = RequestConfig.custom()
                .setConnectTimeout(90000)  // 连接超时时间
                .setSocketTimeout(90000)   // 等待响应超时(读取数据超时)
                .build();
        CloseableHttpClient httpclient = HttpClients.custom()
                .setKeepAliveStrategy(new HttpClientKeepAliveStrategy())
                .setDefaultRequestConfig(globalConfig)
                .setMaxConnTotal(20)      // 同时间正在使用的最多的连接数，默认20
                .setMaxConnPerRoute(10)   // 针对一个域名同时间正在使用的最多的连接数，默认2
                .build();
