...
public class FetcherMgrImpl implements FetcherMgr {
    ...
    public FetcherMgrImpl(RestfulMgr restfulMgr, int retryTime, int retrySleepSeconds,
                          ...
                          String localDownloadDir, String localDownloadDirTemp,
    ...
    @Override
    public ValueVo getChangeFileFromServer(String url) {
        // 远程地址
        RemoteUrl remoteUrl = new RemoteUrl(url, hostList);

        ValueVo valueVo = null;
        try {
            valueVo = restfulMgr.getJsonData(ValueVo.class, remoteUrl, retryTime, retrySleepSeconds);
        } catch (Exception e) {
